...
const Account: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    dispatch(
      SetUpdateStoreUserData(userData.access_token, {
        id: userData.id,
        alias: currentUserData.name,
  ...
  return (
    <ProfileInfo>
      <ChageInfoWrapper>
        {isChange ? (
          <AddPhotoWrapper className="fileContainer">
        ...
        <UserData>
          <InputName
            ...
            onChange={(e: any) => handleChange({ name: 'name', value: e.target.value })}
            size="tiny"
          />
        </UserData>
        <UserDataButtons>
          {isChange ? (
            <Button onClick={handleSubmit} className="ui orange button">
              Save
            </Button>
          ) : (
            <Button onClick={() => setIsChange(!isChange)} className="ui orange button" size="tiny">
              Change
            </Button>
          )}
        </UserDataButtons>
